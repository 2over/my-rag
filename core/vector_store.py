...
class VectorStore:
    def __init__(self, config):
        ...
        if self._is_missing_local_model_path(embed_model):
            raise FileNotFoundError(
                f"Embedding 模型路径不存在: {embed_model}\n"
                "请在项目根目录的 .env 中设置 EMBEDDING_MODEL_NAME，"
                "值可以是本地模型目录，例如 D:\\llm\\Local_model\\BAAI\\bge-large-zh-v1.5，"
                "也可以是 HuggingFace 模型名，例如 BAAI/bge-large-zh-v1.5。"
            )
    ...
    @staticmethod
    def _is_missing_local_model_path(model_name):
        return (
